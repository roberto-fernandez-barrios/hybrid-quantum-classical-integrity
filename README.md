...
monotonicity, closure and materiality results and counterexamples, and the
quantum branch as an instance of the same view lattice; an explicit
adversary model including an executed adaptive attacker; multi-dataset /
fixed-OOD validation with null calibration; a decision-level calibration by a
conformal family rule with an exact finite-sample level; an offline
end-to-end `allow/hold/block` evaluation whose primary endpoint is the number
of materially changed results a policy would serve and whose secondary
endpoint is the cost on benign variation; a bounded quantum-kernel integrity
layer; and an executable contract. No constituent is presented as standalone
...
| Formal core with proofs; statement-by-statement review | `manuscript/FORMAL_CORE.md`, `manuscript/FORMAL_REVIEW_1.3.0.md` |
...
joint-outcome evidence. The conformal family rule calibrates the multi-sensor
regimes to 0.056 / 0.058 / 0.053 (`I_X` / `I_XF` / `I_XFY`; the label-marginal
regime stays at 0.048) against 0.125 / 0.203 / 0.259 for the union of
per-sensor rules; the asymmetric rule of 1.2.0 (0.061 / 0.073 / 0.079) had a
false guarantee (amendment A2), and most of its excess was rule bias, not the
design. Batch-level regimes with feature evidence serve 4,365--4,496 of the
7,008 materially changed results under the calibrated risk-tolerant policy
(the label-marginal regime serves all of them); the trusted item-aligned
regime serves none, with zero clean false actions on its 1,200 exact-zero
rows, but interrupts 52 % of the 1,200 benign near-null controls. The
adaptive cluster-preserving attacker cuts the detection of feature drift from
...
Artifact version: `1.3.0` (frozen 1.0.0 evidence, tag `paper15-q1-v1.0.0`;
...
release tag `paper15-q1-v1.3.0`). Tags `paper15-q1-v1.1.0`,
`paper15-q1-v1.1.1` and `paper15-q1-v1.2.0` and their Zenodo versions are
immutable. After 1.3.0 the article is reopened only for an objective error
that invalidates a claim, an editorial requirement of the journal, or a
reviewer request.
...
version; the version DOI of 1.3.0 is recorded in `CITATION.cff` and
`publication/DOI_STATUS.md` (1.2.0: `10.5281/zenodo.22644529`; 1.1.1:
`10.5281/zenodo.22552643`; 1.1.0: `10.5281/zenodo.22550853`). Author metadata,
CRediT roles and licenses were confirmed by all authors on 2026-09-06 and the
1.3.0 changes on 2026-09-07: code is Apache-2.0, derived evidence and
