...
(This is a first-order diagnostic heuristic; see Section 4.8 for caveats on interpretation.)
...
The recorded run metadata includes `q_shots=1024`; however, because the reported experiments use simulated fidelity-kernel evaluation rather than an explicitly shot-noise-controlled backend experiment, this parameter is retained for configuration traceability and should not be interpreted as an operative hardware-shot setting.


Important implementation note: the experimental configuration records `q_backend_method=statevector` for traceability, but the current `FidelityQuantumKernel` is created with `fidelity=None`. The exact default fidelity primitive is therefore the one selected by `qiskit-machine-learning==0.9.0` when `FidelityQuantumKernel(fidelity=None)` is used. Therefore, the backend object is not force-wired into the kernel primitive. The results should be interpreted as Qiskit Machine Learning fidelity-kernel simulations under the recorded software environment, not as hardware execution or as an explicitly shot-noise-controlled backend experiment.
...
### 5.1.1 Clean operating point

**Table 1. Clean ID performance by model and projected dimension.** Clean performance establishes the operating point from which perturbation impact is measured. Values are computed from the clean rows of the aggregated paper-core results.

| Dimension | Model | Balanced accuracy | ROC-AUC | F1 positive |
|---:|---|---|---|---|
| 8 | QSVC-PauliXYZ | 0.7591 | 0.8281 | 0.7002 |
| 8 | QSVC-Z/PauliXZ-equivalent | 0.7448 | 0.8385 | 0.6809 |
| 8 | QSVC-ZZ | 0.7513 | 0.8499 | 0.7179 |
| 8 | SVC-RBF | 0.7513 | 0.8581 | 0.6684 |
| 10 | QSVC-PauliXYZ | 0.7643 | 0.8266 | 0.7157 |
| 10 | QSVC-Z/PauliXZ-equivalent | 0.7591 | 0.8553 | 0.7067 |
| 10 | QSVC-ZZ | 0.7331 | 0.8550 | 0.7069 |
| 10 | SVC-RBF | 0.7461 | 0.8370 | 0.6627 |
| 12 | QSVC-PauliXYZ | 0.7786 | 0.8475 | 0.7449 |
| 12 | QSVC-Z/PauliXZ-equivalent | 0.7812 | 0.8439 | 0.7515 |
| 12 | QSVC-ZZ | 0.7474 | 0.8855 | 0.7291 |
| 12 | SVC-RBF | 0.7878 | 0.8014 | 0.7324 |

Note that QSVC-ZZ clean balanced accuracy is at or below SVC-RBF at dimensions 10 and 12 (0.7331 vs 0.7461 at dim=10, and 0.7474 vs 0.7878 at dim=12). Therefore, the observed higher perturbation impact for QSVC-ZZ is not attributable to a higher clean starting point.
...
![Figure 1. ID balanced-accuracy impact with 95% seed-unit confidence intervals across unique empirical kernel profiles. The figure supports the main impact claim that QSVC-ZZ shows consistently higher ID impact than SVC-RBF.](../results/paper_figures/paper_core_all_fmaps_plus_baseline/fig12_id_impact_ci_unique_kernel_profiles.png)

**Figure 1.** ID balanced-accuracy impact with 95% seed-unit confidence intervals across unique empirical kernel profiles. The figure supports the main impact claim that QSVC-ZZ shows consistently higher ID impact than SVC-RBF.
...
![Figure 2. ID shared-normalized stealth with 95% seed-unit confidence intervals. Shared normalization enables cross-model comparison by excluding model from the normalization group.](../results/paper_figures/paper_core_all_fmaps_plus_baseline/fig13_id_sharednorm_stealth_ci_unique_kernel_profiles.png)

**Figure 2.** ID shared-normalized stealth with 95% seed-unit confidence intervals. Shared normalization enables cross-model comparison by excluding model from the normalization group.

### 5.3.1 Summary of the main ID comparison

**Table 2. ID comparison between SVC-RBF and QSVC-ZZ using impact and shared-normalized stealth.**

| dim | SVC impact | ZZ impact | impact ratio | impact diff | impact CI95 low | impact CI95 high | SVC stealth | ZZ stealth | stealth ratio | stealth diff | stealth CI95 low | stealth CI95 high |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
Confidence intervals are two-sided 95% Student-t intervals over six paired seed units. They are unadjusted for multiple comparisons across dimensions and are reported to support directional consistency, not as formal per-family hypothesis tests.
...
![Figure 3. Auditability map under ID evaluation. The high-impact/low-detectability quadrant identifies the clearest auditability-gap cases.](../results/paper_figures/paper_core_all_fmaps_plus_baseline/fig11_id_auditability_map_sharednorm.png)

**Figure 3.** Auditability map under ID evaluation. The high-impact/low-detectability quadrant identifies the clearest auditability-gap cases.
...
![Figure 4. Standard feature-centric detectability versus full detectability by perturbation family under ID evaluation. The figure shows that target-shift perturbations are largely invisible to standard feature-centric signals.](../results/paper_figures/paper_core_all_fmaps_plus_baseline/fig9_detectability_old_vs_full_by_family_id.png)

**Figure 4.** Standard feature-centric detectability versus full detectability by perturbation family under ID evaluation. The figure shows that target-shift perturbations are largely invisible to standard feature-centric signals.


## 5.7 OOD acts as a boundary condition

The OOD protocol is interpreted conservatively. Absolute OOD effects are substantially smaller than ID effects. OOD impact-to-SVC ratios are not reported because the SVC-RBF OOD denominator is near zero (approximately 0.0001 to 0.0003 in the relevant dimensions), making ratio comparisons unstable and potentially misleading in this setting., and several confidence intervals include zero. Therefore, OOD results are not used as the main evidence for the central claim.
...
![Figure 5. ID versus OOD shared-normalized stealth at projected dimension 12 with seed-unit confidence intervals. OOD effects are smaller and more variable, supporting the boundary-condition framing.](../results/paper_figures/paper_core_all_fmaps_plus_baseline/fig7_v2_id_vs_ood_sharednorm_stealth_dim12_unique_profiles_ci.png)

**Figure 5.** ID versus OOD shared-normalized stealth at projected dimension 12 with seed-unit confidence intervals. OOD effects are smaller and more variable, supporting the boundary-condition framing.
