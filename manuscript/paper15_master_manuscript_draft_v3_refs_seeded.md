...
Most quantum-kernel studies focus on predictive performance, trainability, expressivity, or the possibility of quantum advantage. This paper addresses a different question: whether benchmark conclusions under quantum kernels are auditable when structured perturbations alter evaluation behaviour.

## 2.3 Quantum feature maps and benchmark sensitivity

Quantum feature maps determine how classical inputs are embedded into quantum Hilbert space. Consequently, the feature map can substantially affect the induced kernel matrix and the resulting classifier behaviour. This makes feature-map-specific reporting essential in quantum-kernel benchmarking.
...
Robustness evaluation studies how model performance changes under noise, corruptions, distribution shift, or other perturbations. Benchmarking under common corruptions has been used to evaluate whether models remain reliable under non-adversarial perturbations [HendrycksDietterich2019]. This paper follows the same general philosophy of controlled stress testing, but applies it to classical and quantum kernel benchmarks.
...
The SVC-RBF baseline uses C=1.0, gamma='scale', and the RBF kernel. The QSVC models use fidelity quantum kernels with reps=1 and statevector simulation. The quantum feature-map configurations are ZZ, Z, PauliXZ, and PauliXYZ, with Z and PauliXZ interpreted as an equivalent empirical kernel profile under the current configuration.
...
The SVC-RBF baseline uses C=1.0, gamma='scale', and the RBF kernel. The QSVC models use `FidelityQuantumKernel`, reps=1, statevector simulation, and the feature-map configurations ZZ, Z, PauliXZ, and PauliXYZ. Z and PauliXZ are interpreted as one equivalent empirical kernel profile under this configuration.
...
This is a seed list. Convert to BibTeX or target-journal format later.
...
## Quantum kernels and QML

[Havlicek2019]
Havlíček, V., Córcoles, A. D., Temme, K., Harrow, A. W., Kandala, A., Chow, J. M., & Gambetta, J. M. Supervised learning with quantum-enhanced feature spaces. Nature / arXiv.

[Schuld2021]
Schuld, M. (2021). Supervised quantum machine learning models are kernel methods. arXiv.

[HuangKuengPreskill2021]
Huang, H.-Y., Kueng, R., & Preskill, J. (2021). Information-theoretic bounds on quantum advantage in machine learning. arXiv.

[Qiskit2024]
Javadi-Abhari, A., et al. (2024). Quantum computing with Qiskit. arXiv.

[QiskitML2025]
Sahin, M. E., et al. (2025). Qiskit Machine Learning: an open-source library for quantum machine learning tasks at scale on quantum hardware and classical simulators. arXiv.

## Robustness, shift, and label noise
...
[FrenayVerleysen2014]
Frénay, B., & Verleysen, M. (2014). Classification in the presence of label noise: a survey. IEEE Transactions on Neural Networks and Learning Systems.

## Drift detection and two-sample testing

[Gretton2008]
Gretton, A., Borgwardt, K. M., Rasch, M. J., Schölkopf, B., & Smola, A. J. (2008/2012). A kernel method for the two-sample problem / A kernel two-sample test. arXiv / JMLR.

[Zliobaite2014]
Žliobaitė, I., Bifet, A., Pechenizkiy, M., & Bouchachia, A. (2014). A survey on concept drift adaptation. ACM Computing Surveys.

## Benchmark reliability and documentation

[Pineau2020]
Pineau, J., et al. (2020). Improving Reproducibility in Machine Learning Research. arXiv.

[Gebru2021]
Gebru, T., et al. Datasheets for Datasets. Communications of the ACM.

[Mitchell2019]
Mitchell, M., et al. (2019). Model Cards for Model Reporting. FAT* / FAccT.

## Dataset / security context

[Sharafaldin2018]
Sharafaldin, I., Lashkari, A. H., & Ghorbani, A. A. (2018). Toward Generating a New Intrusion Detection Dataset and Intrusion Traffic Characterization. CICIDS2017-related reference.
