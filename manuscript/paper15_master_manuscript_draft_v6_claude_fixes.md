...
The paper-core experiments were run with `q_max_iter=1000` in the experiment launcher. This value is reported as the run configuration used to generate the paper CSVs. The implementation also contains other defaults, such as run-grid defaults and dataclass fallback values, but those defaults should not be interpreted as the configuration used for the reported paper-core runs.
