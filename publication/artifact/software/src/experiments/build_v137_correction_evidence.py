...
def _headline(
    ...
    add(
        "Gate A adaptive P2 material-served headline range",
        "28--39%",
        "27--39%",
        "lower endpoint -1 pp after two-decimal reporting",
        "same 3,418 material Gate-A adaptive rows in each batch regime; corrected JSD only",
    )
...
def build(repo: Path, raw_dir: Path, jsd_dir: Path, label_dir: Path) -> None:
    ...
    # Both frozen calibration sensitivities are deterministic transformations of
    # the same corrected clean rows.  Re-derive them because their JSD coordinates
    # changed; this is not a new calibration campaign or a retroactive primary.
    corrected_cal = null_new_s[null_new_s["attack_priority_group"] == "null_calibration"]
    corrected_eval = null_new_s[null_new_s["attack_priority_group"] == "null_evaluation"]
    resplit_cells, resplit_pooled, resplit_ok = _resplit_decomposition(corrected_cal, corrected_eval)
    split_sensitivity = _split_sensitivity(corrected_cal, corrected_eval)
    old_resplit = pd.read_csv(evidence / "policy/family_resplit_pooled.csv")
    old_split = pd.read_csv(evidence / "policy/family_split_construction_sensitivity.csv")
    resplit_delta = old_resplit.merge(
        resplit_pooled,
        on="regime",
        suffixes=("__previous", "__corrected"),
        validate="one_to_one",
    )
    split_delta = old_split.merge(
        split_sensitivity,
        on="regime",
        suffixes=("__previous", "__corrected"),
        validate="one_to_one",
    ...
    for regime in BATCH_REGIME_NAMES:
        old_r = old_resplit.set_index("regime").loc[regime]
        new_r = resplit_pooled.set_index("regime").loc[regime]
        old_s = old_split.set_index("regime").loc[regime]
        new_s = split_sensitivity.set_index("regime").loc[regime]
        headline = pd.concat(
            [
                headline,
                pd.DataFrame.from_records(
                    [
                        {
                            "claim": f"Exchangeable re-split conformal response ({regime})",
                            "v1.3.6": f"{float(old_r['resplit_rate_family']):.3f}",
                            "v1.3.7": f"{float(new_r['resplit_rate_family']):.3f}",
                            "delta": f"{float(new_r['resplit_rate_family']) - float(old_r['resplit_rate_family']):+.3f}",
                            "reason": "same 30 frozen permutations and seed; corrected JSD coordinates only",
                        },
                        {
                            "claim": f"Split-construction conformal clean response ({regime})",
                            "v1.3.6": f"{int(old_s['n_fire'])}/12000 ({float(old_s['pooled_rate']):.3f})",
                            "v1.3.7": f"{int(new_s['n_fire'])}/12000 ({float(new_s['pooled_rate']):.3f})",
                            "delta": int(new_s["n_fire"]) - int(old_s["n_fire"]),
                            "reason": "same frozen reference/calibration/evaluation rows; corrected JSD coordinates only",
                        },
                    ]
                ),
            ],
    ...
    outputs = {
        ...
        "jsd_corrected_resplit_by_cell.csv": resplit_cells,
        "jsd_corrected_resplit_pooled.csv": resplit_pooled,
        "jsd_corrected_split_construction_sensitivity.csv": split_sensitivity,
        "jsd_resplit_delta.csv": resplit_delta,
        "jsd_split_construction_delta.csv": split_delta,
    ...
    checks: dict[str, bool] = {
        ...
        "corrected_exchangeable_resplits_within_declared_tolerance": bool(resplit_ok),
