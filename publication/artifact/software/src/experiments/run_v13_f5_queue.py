...
def main() -> None:
    ...
    for r in results:  # record repository-relative log paths (no machine-specific paths in the status file)
        if isinstance(r.get("log"), str):
            try:
                r["log"] = Path(r["log"]).resolve().relative_to(repo).as_posix()
            except ValueError:
                pass
