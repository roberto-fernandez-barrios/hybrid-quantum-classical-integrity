...
def verify(repo: Path) -> dict[str, object]:
    ...
    changed_observation_rows = len(deltas[["correction_scope", "affected_row_identifier"]].drop_duplicates())
    if changed_observation_rows != 58074 or len(deltas) != 192487:
        raise ValueError("correction delta cardinalities changed")
    ...
    split = pd.read_csv(jsd_dir / "jsd_corrected_split_construction_sensitivity.csv")
    if len(resplit) != 4 or len(split) != 4 or bool((resplit["resplit_rate_family"] > resplit["conformal_level"] + 0.005).any()):
        raise ValueError("corrected frozen calibration sensitivities are incomplete or out of tolerance")
    ...
    result = {
        ...
        "changed_observation_rows": changed_observation_rows,
