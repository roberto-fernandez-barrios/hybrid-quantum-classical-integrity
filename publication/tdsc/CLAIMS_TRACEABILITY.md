...
| Null-calibrated feature and feature-plus-prediction regimes never fire on evaluation-label interventions; label-marginal regime never fires on prior-preserving interventions | Exact invariance survives calibration (1.1.0) | `evidence/reinforcement/calibrated_label_path_summary.csv`; `null_calibration_thresholds.csv` | Verifier recomputes zero firings on all 3,600 label rows; thresholds from calibration draws only |
| Empirical false-alarm and detection rates of non-invariant sensors at alpha = 0.05 in the frozen design | Empirical, fixed design, simulator | `null_evaluation_false_alarm_*.csv`; `calibrated_detection_*.csv`; `fig_q1_calibrated_coverage.pdf` | Manifest checks 200 calibration and 200 disjoint evaluation draws per cell and pool disjointness |
| The secondary ZZ-minus-SVC profile under matched scalers and under cross-validated tuning of both learners | Secondary sensitivity | `prep_ablation_paired_zz_minus_svc.csv`; `tuned_paired_zz_minus_svc.csv`; `tuned_selected_hyperparameters*.csv` | Split-cluster intervals; exact label-path invariance re-verified in every configuration |
| Artifact files and counts have not changed silently | Integrity of released evidence | `publication/artifact/ARTIFACT_MANIFEST.sha256` plus five embedded manifests | `verify_publication_artifact` recomputes hashes, shapes, and primary counts |
