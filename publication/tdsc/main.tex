...
\ifdefined\pdfinfo
  \pdfinfo{
    ...
    /Subject (Targeted methodological sensitivity amendment for IEEE Transactions on Dependable and Secure Computing)
...
% TDSC does not offer double-anonymous review. Author block and release v1.3.5 are author-approved.
...
indistinguishability under declared views and references. The constructive
result is claim-relative minimum evidence granularity within the declared
reference lattice: a trusted aggregate for the same batch suffices for
conclusion integrity, item-identity integrity requires item-aligned evidence,
and an external trust
root remains necessary. Across eight frozen environments from CICIDS2017,
UNSW-NB15, and ToN-IoT, 3,600 label interventions confirm the structural
boundaries. The original design calibrates with clean resamples but scores
interventions against a protected clean same-item set. A preregistered
geometry-aligned sensitivity fixes the reference and batch construction
between clean and intervention. Mean shift and scaling remain almost fully
responsive; dropout remains strength-dependent. The adaptive cluster-preserving
intervention reduces response against matched controls in 25--40 of 40
environment/split cells per comparison while retaining materially altered
audit results. Thus calibration controls clean-resample false actions but not
attack coverage. A full-conformal Max-Rank family rule is valid under
exchangeability; executed false-action rates are descriptive. A bounded
165-cell ideal-statevector and finite-shot-emulation gate maps the distinctions
onto circuit, kernel, and estimation boundaries without a hardware claim.
\end{abstract}
...
granularity within the declared reference lattice of workflow-wide information sets that includes the
...
relabeling. These are minimum levels within the declared reference lattice, up
to informational equivalence, not minima over every possible encoding or audit
architecture.
...
The v1.3.5 sensitivity leaves frozen evidence intact and replays the same 240
...
decision logic, not throughput. It is the present research instance of the
...
The frozen original Gate~A used clean-resample calibration but scored attacked
...
design-specific, not evidence against a generally powerful detector.
...
deployment prevalence. In the original Gate~A design, 28--39\% of material
instances are served in aggregate. The aligned sensitivity preserves the
control/adaptive ordering but raises adaptive response, showing both a geometry
effect and residual monitor-aware evasion. Calibration controls clean-resample
false actions under its declared construction, not what an attacker can hide.

The claim-relative minimum is within the declared reference lattice:
granularity is authenticated information, not representation length. A compact
Merkle root or position-binding vector commitment can still bind item-aligned
information; level C does not collapse into level B
\cite{Merkle1988,CatalanoFiore2013}. A trusted entity can authenticate $M_0$
from aligned pairs at reference-generation time; at audit time its anchor can
be checked without retaining the item-wise label store. Conclusion integrity
needs aggregate information, item identity needs alignment, and an external
root remains necessary (Proposition~6). This trust/reference model is neither a
deployed protocol nor a minimum over every possible audit architecture.

The trusted cost is likewise claim-relative and specific to the original
frozen benchmark design. Its
...
The eight fixed settings are not sampled deployments: split clusters measure
within-environment resampling, most gates use 128/128 samples, and balanced
numeric staging does not reproduce prevalence. The quantum evaluators are
ideal statevector simulators with finite-shot emulation, not hardware evidence.
SVC and QSVC retain pipeline-specific scalers, so their comparison does not
isolate kernel geometry. Exact blindness under constructed perturbations is a
capability result, not a prevalence estimate.

The conformal level is marginal and requires exchangeability; overlapping
draws, different pool rows, and E1 violate that premise. We report the executed
excess and claim no conditional guarantee for a fixed calibration set. Drift,
near-null response and adaptive evasion share local cluster structure. The
sensitivity aligns geometry but cannot causally apportion that structure,
same-item editing and evasion. Gate~A fixes one rule and two mechanisms.

The trusted item-aligned regime is an assumption: a rewritable reference falls
under Proposition~6, and its measured cost is not operational traffic. P3
abstains only on missing coverage, not low power or invalid calibration. The
prototype's hash chain establishes consistency, not provider authenticity;
every policy result is offline. Runtime non-stationarity, recovery, real-QPU
execution and service-level enforcement require a separate study.
...
these cases and their near-null cost. The geometry-aligned sensitivity
preserves the matched control/adaptive ordering while bounding the original
same-item effect; the conformal guarantee remains
conditional on exchangeability and the quantum validation simulator-bounded.
The constructive result is claim-relative minimum evidence within the declared
reference lattice: an authenticated
...
The review artifact contains code, tests, derived evidence, nine manifests,
preregistrations and replay commands. Raw benchmark data are not redistributed;
sources, hashes and staging are documented. Original evidence is frozen at 1.3.0
(version DOI 10.5281/zenodo.22648573). Version
1.3.5 is archived at Zenodo, version DOI 10.5281/zenodo.22678092 (concept DOI
...
The authors used OpenAI Codex and Anthropic Claude Code for drafting/language
editing, code generation/inspection, formal checking, literature support, and
artifact/release preparation across Methods, Results, formal discussion,
supplement and documentation. Humans defined and approved every protocol and
claim, verified all sources, proofs, code, results and generated text, and
assume full responsibility. The submission package gives system-by-section
detail.
